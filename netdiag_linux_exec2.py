    ...
    if users and pwds:
        for u in users:
            for p in pwds:
                ...
                if ok:
                    return u, p
        raise RuntimeError("No valid SSH credentials found")
...
def run_tool(call: dict) -> str:
    ...
    if tool == "ping":
        ...
        if "host" not in args:
            return "ERROR: ssh_exec missing required argument: host"

        if "commands" not in args:
            return "ERROR: ssh_exec missing required argument: commands"

        if not isinstance(args["commands"], list):
            return "ERROR: ssh_exec 'commands' must be a list of strings"
...
def model_loop(history):
    while True:
        ...
        # Prevent tool calls for explanation-style prompts
        if any(w in history[-1]["content"].lower()
               for w in ("explain", "what is", "how does", "describe")):
            print("\nAssistant:", content)
